...
void MainWindow::on_pushButton_write_clicked()
{
    ...
    ispd_week = m_time->week_of_year(this->m_year, this->m_week, this->m_day);
    ...
    for(int i = this->m_start_id; i < (this->m_start_id + this->m_ispd_num); i++)
    {
        ...
        QString ser_number = ispd_year + ispd_week + ispd_day + ui->comboBox_type->currentText() +
...
{
    m_xls->setCurrentSheet(this->m_execl_sheet);
    m_xls->write(1, 1, tr("超级版原始ID"));
    m_xls->write(1, 2, tr("超级版加密ID"));
    m_xls->write(1, 3, tr("产品型号"));
    m_xls->write(1, 4, tr("产品颜色"));
    m_xls->write(1, 5, tr("电池类型"));
    m_xls->write(1, 6, tr("防水等级"));
    m_xls->write(1, 7, tr("输入电压"));
    m_xls->write(1, 8, tr("输入电流"));
    m_xls->write(1, 9, tr("电池容量"));
    m_xls->write(1, 10, tr("序列号"));
    m_xls->write(1, 11, tr("日期时间"));
    m_xls->save();
